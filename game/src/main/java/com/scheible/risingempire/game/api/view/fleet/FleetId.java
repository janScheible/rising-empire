...
/**
 * Unique (if enforced somehow from the outside) identifier of a fleet. It is a hex number in the interval
 * [0x123456, 0xFFFFFF).
 * 
 * @author sj
 */
public class FleetId {
	...
	public FleetId(final String value) {
		this.value = Long2.tryParseLong(value, 16).filter(n -> n >= MIN && n < MAX).map(n -> value)
				.orElseThrow(() -> new IllegalArgumentException(
						String.format("Id must be a valid hex number in the interval [0x%s, 0x%s)!",
