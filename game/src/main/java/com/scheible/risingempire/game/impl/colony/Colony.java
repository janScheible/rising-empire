...
public class Colony {
	...
	static void adjustRationInternal(final ProductionArea area, final int percentage,
		...
		if (difference > 0) {
			while (difference != 0) {
				final Entry<ProductionArea, Integer> highestOtherRatio = ratios.entrySet().stream()
						.filter(areaAndAmount -> areaAndAmount.getKey() != area)
						.sorted((a, b) -> Integer.compare(b.getValue(), a.getValue())).findFirst().get();
			...
			final Entry<ProductionArea, Integer> lowestOtherRatio = ratios.entrySet().stream()
					.filter(areaAndAmount -> areaAndAmount.getKey() != area)
					.sorted((a, b) -> Integer.compare(a.getValue(), b.getValue())).findFirst().get();
	...
	public String toString() {
		return toStringBuilder(getClass()).add("player", player).add("population", population)
				.add("spaceDock", spaceDock).add("ratios", ratios).toString();
	}
}
