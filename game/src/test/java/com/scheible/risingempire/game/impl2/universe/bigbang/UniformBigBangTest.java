...
class UniformBigBangTest {
	...
	void testHugeBigBang() throws IOException {
		...
		Set<Location> locations = new UniformBigBang().systemLocations(galaxySize, 200, new SeededRandom(93466547));
	...
	void testSameSeedSameResult() {
		Set<Location> firstLocations = new UniformBigBang().systemLocations(GalaxySize.HUGE, 200,
				new SeededRandom(93466547));
		Set<Location> secondLocations = new UniformBigBang().systemLocations(GalaxySize.HUGE, 200,
				new SeededRandom(93466547));

		assertThat(firstLocations).containsAll(secondLocations);
	}
