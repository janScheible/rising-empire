...
class FieldUtilsTest {
	...
	void testViewportParsing() {
		assertThat(FieldUtils
			.getViewport(singletonList("$.starMap.stars[?(@.x > 0 && @.x < 1003 && @.y > 19 && @.y < 500)]"), "stars"))
			.isPresent()
			.hasValue(new FieldUtils.Viewport(0, 1003, 19, 500));
	}

	@Test
	void testWrongType() {
		assertThat(FieldUtils
			.getViewport(singletonList("$.starMap.stars[?(@.x > 0 && @.x < 1003 && @.y > 19 && @.y < 500)]"), "fleets"))
			.isEmpty();
	}
