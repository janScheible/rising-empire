...
class SpaceCombatFakeFleetsIT extends AbstractMainPageIT {
	...
	void testFakeFleetForLostOne() throws Exception {
		...
		assertThat(extractFleets(blueClient)).hasSize(5).areExactly(1, fleet("blue", 60, 60))
				.areExactly(1, fleet("blue", 220, 100)).areExactly(1, fleet("white", 220, 100))
				.areExactly(1, fleet("blue", 140, 340)).areExactly(1, fleet("yellow", 140, 340));
		...
		assertThat(blueClient).is(mainPageState("SpaceCombatSystemState"));
		assertThat(extractFleets(blueClient)).hasSize(4).areExactly(1, fleet("blue", 60, 60))
				.areExactly(1, fleet("blue", 220, 100)).areExactly(1, fleet("blue", 140, 340))
		...
		blueClient.submit("$._actions[?(@.name=='continue')]");
		assertThat(blueClient).is(mainPageState("NewTurnState"));
		assertThat(extractFleets(blueClient)).hasSize(3).areExactly(1, fleet("blue", 60, 60))
				.areExactly(1, fleet("blue", 220, 100)).areExactly(1, fleet("blue", 140, 340));
	}
}
