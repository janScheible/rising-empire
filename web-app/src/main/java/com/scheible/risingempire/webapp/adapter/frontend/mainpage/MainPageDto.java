		...
		if (optionalFields.isPresent()) {
			...
			if (mainPage.starMap != null) {
				if (anyFieldStartsWith.test("$.starMap.stars")) {
					...
					mainPage.starMap.getContent().stars = mainPage.starMap.getContent().stars.stream().filter(
							s -> viewport.isEmpty() || viewport.get().contains(s.getContent().x, s.getContent().y))
				...
				if (anyFieldStartsWith.test("$.starMap.fleets")) {
					...
					mainPage.starMap.getContent().fleets = mainPage.starMap.getContent().fleets.stream().filter(
							f -> viewport.isEmpty() || viewport.get().contains(f.getContent().x, f.getContent().y))
							.collect(Collectors.toList());
